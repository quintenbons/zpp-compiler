...
void Type::debug(size_t depth) const {
...
void Variable::debug(size_t depth) const {
  logNode(depth, name);
  if (description)
...
void NumberLiteral::debug(size_t depth) const { logNode(depth, number); }

void StringLiteral::debug(size_t depth) const { logNode(depth, content); }

void Expression::debug(size_t depth) const {
  std::visit([depth](const auto &node) { node.debug(depth); }, expr);
}

void FunctionCall::debug(size_t depth) const {
...
void Declaration::debug(size_t depth) const {
...
void ReturnStatement::debug(size_t depth) const {
  logNode(depth);
  expression.debug(depth);
}

void InlineAsmStatement::debug(size_t depth) const {
...
void Instruction::debug(size_t depth) const {
  std::visit([depth](const auto &node) { node.debug(depth); }, instr);
}

void InstructionList::debug(size_t depth) const {
...
void FunctionParameter::debug(size_t depth) const {
  logNode(depth, "Type: ", type.fullName(), " ; Name: ", name);
}

void Attribute::debug(size_t depth) const {
  logNode(depth, "Type: ", type.fullName(), " ; Name: ", name);
}

void FunctionParameterList::debug(size_t depth) const {
...
void Function::debug(size_t depth) const {
...
void Method::debug(size_t depth) const {
  logNode(depth, "ReturnType: ", returnType.fullName(), " ; Name: ", name,
          " ; ParamCount: ", params.size());
  returnType.debug(depth + 1);
  params.debug(depth + 1);
...
void AccessSpecifier::debug(size_t depth) const {
  switch (level) {
...
void Class::debug(size_t depth) const {
...
void TranslationUnit::debug(size_t depth) const {
